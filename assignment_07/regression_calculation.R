...
#Enter into last matrix, b



A <- matrix(c(n, sum(x), sum(x), sum(x^2)),
            nrow = 2,
            ncol = 2)
...
beta_hat_norm <- A_inv %*% b



#above & below should give the same answer
...
ssr <- function(beta,y,x){
        ssr <- sum((y-beta_0_hat_calc-(beta_1_hat_calc*x))^2)
        return(ssr)
}
